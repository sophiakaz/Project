...
public class Receive implements Interface {
    ...
    public Receive(byte[] key, int[] ct, int[] pt){
    	setupKey(key);
    	decrypt(ct,pt);
    }
    
    public void decrypt(int[] ct, int[] pt){ // 2 WORD input ct/output pt
        int i;
        int B = ct[1];
        int A = ct[0];
        for (i = r; i > 0; i--){
    ...
    public void encrypt(int[] pt, int[] ct){ // 2 WORD input pt/output ct
        int i;
        int A = pt[0] + S[0];
        int B = pt[1] + S[1];
        for (i = 1; i <= r; i++){
